...
public class CircularBuffer {
    ...
    public int availableWrite() {
        return BUFFER_SIZE - this.availableRead();
    }

    private int getReadPos() {
        return (int) (this.readPos % BUFFER_SIZE);
    }

    private int getWritePos() {
        return (int) (this.writePos % BUFFER_SIZE);
    }
    ...
    public int read(byte[] b, int off, int len) {
        ...
        synchronized (this) {
            ...
            while (remaining > 0) {
                ...
                while (available > 0) {
                    int toRead = Math.min(available, BUFFER_SIZE - this.getReadPos());
    ...
    public int write(byte[] b, int off, int len) {
        synchronized (this) {
            ...
            while (remaining > 0) {
                ...
                while (available > 0) {
                    int toWrite = Math.min(available, BUFFER_SIZE - this.getWritePos());
